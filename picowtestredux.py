...
PACKET_SAMPLES = 320                 # samples per packet = 20 ms at 16 kHz
                                     # (20 ms is a good trade-off)
...
# Broadcast address for AP subnet (clients get 192.168.4.x)
...
while True:
    ...
    for i in range(PACKET_SAMPLES):
        ...
        now = time.monotonic()
        if target > now:
            time.sleep(target - now)         # sleep most of the interval
        # (If we wake up late, we just continue – this introduces a tiny
        #  amount of jitter but keeps the loop from falling behind)
